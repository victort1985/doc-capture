...
import 'screens/terms_of_service_gate_screen.dart';
...
class DocCaptureApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final banksService = BanksService(apiService);
    ...
    return MultiProvider(
      providers: [
        ...
        Provider<BanksService>.value(value: banksService),
    ...
    return MaterialApp(
      ...
      home: FutureBuilder<void>(
        ...
        builder: (context, snapshot) {
          ...
          // TOS acceptance and organization confirmation are both
          // enforced HERE, at the top-level routing decision — not as
          // one-shot checks inside login_screen.dart's own submit
          // handler (the earlier, fragile approach for org-picking,
          // documented in this same comment historically: it only ran
          // from that single call site, and anything else that set
          // currentUser a different way silently skipped it). Now:
          // logged in but tosAccepted is still false means RootScreen
          // is structurally unreachable, no matter how currentUser
          // got set, exactly like org-confirmation below.
          final needsTos = loggedIn && !(appState.currentUser?.tosAccepted ?? false);
          final needsOrgPick = loggedIn && !needsTos && !appState.orgConfirmed;
          ...
          return LicenseGate(
            child: _EulaGate(
              ...
              child: !loggedIn
                  ? const LoginScreen()
                  : needsTos
                      ? TermsOfServiceGateScreen(language: appState.currentUser?.language ?? 'he')
                      : needsOrgPick
