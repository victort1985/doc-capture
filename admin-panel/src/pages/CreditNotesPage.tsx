...
import CreditNoteSettingsPage from './CreditNoteSettingsPage';
...
export default function CreditNotesPage() {
  ...
  async function load() {
    ...
    try {
      setNotes(await apiFetch<CreditNoteRow[]>('/credit-notes'));
    } catch (e) {
  ...
  async function viewPdf(id: number) {
    try {
      const url = await apiFetchBlob(`/credit-notes/${id}/pdf`);
      window.open(url, '_blank');
    } catch (e) {
      alert(e instanceof Error ? e.message : t('creditNotes.noPdf'));
    }
  }
  ...
  return (
    <div className="page">
      <div className="topbar">
        <div><div className="eyebrow">{t('creditNotes.eyebrow')}</div><h1>{t('creditNotes.title')}</h1></div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button type="button" onClick={() => setShowCreate(true)}><Plus size={15} /> {t('creditNotes.create')}</button>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border, #e5e5e5)' }}>
              <th style={{ padding: '8px 12px' }}>{t('creditNotes.number')}</th>
              <th style={{ padding: '8px 12px' }}>{t('creditNotes.client')}</th>
              <th style={{ padding: '8px 12px' }}>{t('creditNotes.reason')}</th>
              <th style={{ padding: '8px 12px' }}>{t('creditNotes.invoice')}</th>
              <th style={{ padding: '8px 12px' }}>{t('creditNotes.total')}</th>
            ...
            {notes.map((n) => (
              <tr key={n.id} style={{ borderBottom: '1px solid var(--border, #f0f0f0)' }}>
                ...
                <td style={{ padding: '8px 12px' }}>
                  <button type="button" onClick={() => viewPdf(n.id)} title={t('creditNotes.viewPdf')}><FileText size={15} /></button>
                </td>
              </tr>
            ))}
            {notes.length === 0 && !loading && (
              <tr><td colSpan={6} style={{ padding: '16px 12px', color: 'var(--ink-soft)' }}>{t('creditNotes.empty')}</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function CreateCreditNoteModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  function selectInvoice(inv: InvoiceOption) {
    ...
    setItems([{ description: t('creditNotes.defaultItemDescription', { number: inv.invoiceNumber ?? `#${inv.id}` }), quantity: 1, unitPrice: inv.total }]);
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={onClose}>
      <div className="card" style={{ width: 520, maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h2 style={{ margin: 0 }}>{t('creditNotes.createTitle')}</h2>
          <button className="ghost" onClick={onClose}><X size={16} /></button>
        </div>

        {!selectedInvoice ? (
          <>
            <label>{t('creditNotes.pickInvoiceLabel')}</label>
            <input type="text" value={invoiceQuery} onChange={e => setInvoiceQuery(e.target.value)} placeholder={t('creditNotes.searchPlaceholder')} style={{ width: '100%', marginBottom: 10 }} />
            <div style={{ maxHeight: 260, overflowY: 'auto', border: '1px solid var(--border, #e5e5e5)', borderRadius: 8 }}>
              ...
              {filteredInvoices.length === 0 && <div style={{ padding: 12, color: 'var(--ink-soft)' }}>{t('creditNotes.noInvoicesFound')}</div>}
            </div>
          </>
        ) : (
          <>
            <div style={{ marginBottom: 10, fontSize: 13.5 }}>
              {t('creditNotes.correcting')}: <b>{selectedInvoice.invoiceNumber ?? `#${selectedInvoice.id}`}</b> · {selectedInvoice.clientName} · ₪{selectedInvoice.total.toFixed(2)}
              <button className="ghost" style={{ marginInlineStart: 8 }} onClick={() => setSelectedInvoice(null)}>{t('creditNotes.changeInvoice')}</button>
            </div>
            <label>{t('creditNotes.reasonLabel')}</label>
            <textarea value={reason} onChange={e => setReason(e.target.value)} rows={2} style={{ width: '100%', marginBottom: 10 }} placeholder={t('creditNotes.reasonPlaceholder')} />

            {items.map((item, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <input type="text" value={item.description} onChange={e => setItems(prev => prev.map((it, idx) => idx === i ? { ...it, description: e.target.value } : it))} placeholder={t('creditNotes.itemDescription')} style={{ flex: 2 }} />
            ...
            <button type="button" className="ghost" onClick={() => setItems(prev => [...prev, { description: '', quantity: 1, unitPrice: 0 }])} style={{ marginBottom: 12 }}>
              + {t('creditNotes.addItem')}
            </button>

            <div style={{ fontWeight: 700, marginBottom: 12 }}>{t('creditNotes.total')}: ₪{total.toFixed(2)}</div>
            {error && <div className="error-banner" style={{ marginBottom: 10 }}>{error}</div>}
            <button type="button" disabled={saving || !reason.trim() || total <= 0} onClick={submit} style={{ width: '100%' }}>
              {saving ? t('common.saving') : t('creditNotes.submit')}
            </button>
          </>
        )}
      </div>
