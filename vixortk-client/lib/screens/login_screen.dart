...
class _LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _stampController = AnimationController(vsync: this, duration: const Duration(milliseconds: 520));
    _stampScale = Tween<double>(begin: 1.6, end: 1.0).animate(CurvedAnimation(parent: _stampController, curve: Curves.easeOutBack));
    _stampRotation = Tween<double>(begin: -0.12, end: 0.0).animate(CurvedAnimation(parent: _stampController, curve: Curves.easeOutCubic));
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // Nothing else to do — main.dart's own top-level routing
      // (reactively watching AppState) shows RootScreen once
      // currentUser is set. VixorTK has no ToS/setup-wizard/org-picker
      // gates to route through first, unlike the main Vixor ERP app.
    } on DioException catch (e) {
      if (!mounted) return;
      final l10n = AppLocalizations.of(context)!;
      // Same distinction as the main app's own login screen: a 401 is
      // genuinely a wrong password; anything else (no connection, TLS
      // failure, timeout, wrong address) is a reachability problem and
      // showing "wrong username or password" for those would be
      // actively misleading.
      if (e.response?.statusCode == 401) {
        ...
        final detail = status != null ? 'HTTP $status' : (e.error?.toString() ?? e.message ?? e.type.name);
            ...
            children: [
                        ...
                        children: [
                          Center(
                            child: AnimatedBuilder(
                              ...
                              child: ClipRRect(
                                borderRadius: BorderRadius.circular(18),
                                child: Image.asset('assets/icons/app-icon.png', width: 76, height: 76),
                              ),
                            ),
                          ),
                          ...
                          RichText(
                            ...
                            text: TextSpan(children: [
                              TextSpan(text: 'VIXOR', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 26, letterSpacing: 4, color: Theme.of(context).colorScheme.onSurface)),
                              const TextSpan(text: ' TK', style: TextStyle(fontWeight: FontWeight.w300, fontSize: 22, letterSpacing: 2, color: Color(0xFFF2701C))),
                            ]),
                          ),
                          const SizedBox(height: 4),
                          Text(l10n.loginTitle, textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodySmall),
                          const SizedBox(height: 26),
                          TextField(
                            controller: _usernameController,
                            decoration: InputDecoration(labelText: l10n.username, prefixIcon: const Icon(Icons.person_outline, size: 20)),
                          ),
                          ...
                          if (_needsTotp) ...[
                            ...
                            TextField(
                              ...
                              maxLength: 6,
                              decoration: InputDecoration(labelText: l10n.totpCode, prefixIcon: const Icon(Icons.shield_outlined, size: 20), counterText: ''),
                            ),
                          ],
              ...
              Positioned(
                ...
                child: IconButton(
                  ...
                  onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const ConnectionSettingsScreen())),
                ),
              ),
            ],
          ),
