...
export function buildDocumentLineRecord(input: DocumentLineInput): string {
  const fields = [
    ...
    signedAmountField(input.quantity, 17, 4), // 1264 — CONFIRMED via a REAL official example BKMVDATA.TXT file (obtained directly from the Tax Authority's own site) rather than derived from this app's own output: an example line for quantity=100/unitPrice=3000.00/discount=-15000.00/lineTotal=285000.00 only reconciles (100*3000-15000=285000, exact) when quantity is decoded with 4 decimal places, not 2. The previous commit's "2 decimals" conclusion was a circular check — it confirmed this app's OWN written quantity/price/total agreed with EACH OTHER under the same (wrong) 2-decimal assumption, which is self-consistent but doesn't prove anything about what the real simulator expects when it decodes the same bytes independently. 17 chars = 1 sign + 12 integer + 4 decimal.
