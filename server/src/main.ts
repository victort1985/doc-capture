...
async function bootstrap() {
  ...
  if (corsOrigin) {
    app.enableCors({ origin: corsOrigin.split(',').map((o) => o.trim()), exposedHeaders: ['X-Bkmvdata-Size-Bytes', 'X-Exceeds-Simulator-Limit', 'X-Vat-Checksum-Valid'] });
  } else {
    ...
    app.enableCors({
      ...
      exposedHeaders: ['X-Bkmvdata-Size-Bytes', 'X-Exceeds-Simulator-Limit', 'X-Vat-Checksum-Valid'],
    });
    // eslint-disable-next-line no-console
    console.warn(
