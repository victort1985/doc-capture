...
## 6. Мульти-тенантный режим (несколько фирм-клиентов на одном сервере)

Каждая фирма-клиент получает полностью отдельную БД и отдельный процесс сервера (не просто фильтрацию по `organizationId` внутри общей базы) — так исключается сам факт "перепутанных" данных между фирмами, даже при баге. Все процессы используют один и тот же собранный код (`dist/`), различается только `.env`.

**Новая фирма:**
```bash
export LICENSE_ADMIN_USER=<твой логин на сервере лицензий>
export LICENSE_ADMIN_PASS=<пароль>
sudo ./server/scripts/provision-tenant.sh company-a "Company A Ltd" 4101 10
```
Создаёт БД, `.env`, лицензионный ключ (с лимитом устройств), инициализирует схему, поднимает `doc-capture@company-a.service`.

**Обновление кода — сразу для всех фирм:**
```bash
sudo ./server/scripts/deploy-all-tenants.sh
```
Собирает один раз, перезапускает все запущенные инстансы (и старый `doc-capture.service`, если ещё используется, и все `doc-capture@<slug>`).
